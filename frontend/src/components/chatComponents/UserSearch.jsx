...
const UserSearch = () => {
	...
	useEffect(() => {
		const getAllUsers = () => {
			...
			fetch(`${(import.meta.env.VITE_BACKEND_URL || "http://localhost:9000")}/api/user/users`, {
				method: "GET",
				headers: {
	...
	const handleCreateChat = async (userId) => {
		const token = localStorage.getItem("token");
		fetch(`${(import.meta.env.VITE_BACKEND_URL || "http://localhost:9000")}/api/chat`, {
			method: "POST",
			headers: {
				"Content-Type": "application/json",
				Authorization: `Bearer ${token}`,
			...
			.catch((err) => {
				console.log(err);
				toast.error(err?.message || "Failed to create/open chat");
				dispatch(setLoading(false));
			});
	};
	return (
